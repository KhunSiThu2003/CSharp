    ...
    public partial class VerifyEmail : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("~/AuthPages/JobSeeker/Register.aspx", true);
                    return;
                }
        ...
        protected void btnVerify_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    var (jobSeekerId, storedCode, expiryDate) = GetVerificationData(email, connection);
                    ...
                    {
                        ActivateUser(jobSeekerId, connection);
                        ClearVerificationSession();
                        Session["IsAuthenticated"] = true;
                        Session["JobSeekerId"] = jobSeekerId;
                        Response.Redirect("~/Pages/JobSeeker/Dashboard.aspx", true);
                    }
                }
            }
            catch (Exception ex)
        ...
        private (string jobSeekerId, string code, DateTime expiry) GetVerificationData(string email, SqlConnection connection)
        {
            string query = @"SELECT JobSeeker_Id, VerificationCode, VerificationCodeExpiry 
                           FROM JobSeeker WHERE Email = @Email";
            ...
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        return (
                            reader.GetString(0),  // JobSeeker_Id
        ...
        {
            string query = @"SELECT VerificationAttempts FROM JobSeeker WHERE Email = @Email";
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                command.Parameters.AddWithValue("@Email", email);
        ...
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333333; line-height: 1.6;'>
            ...
            <div style='margin-bottom: 30px;'>
                <h2 style='color: #1e293b; margin: 0 0 20px 0; font-size: 18px; text-align: center;'>Complete Your Registration</h2>
                <p style='margin: 0 0 20px 0;'>
                    Thank you for creating a JobConnect account! To get started, please verify your email address by entering the following verification code:
                </p>
                ...
                <p style='margin: 0; color: #64748b;'>
                    If you didn't create a JobConnect account, you can safely ignore this email.
                </p>
            </div>
            ...
            <div style='border-top: 1px solid #e2e8f0; padding-top: 20px; text-align: center; font-size: 12px; color: #94a3b8;'>
                ...
                using (MailMessage message = new MailMessage())
                {
                    ...
                    message.Subject = "Verify Your JobConnect Account";
        ...
        private void ActivateUser(string jobSeekerId, SqlConnection connection)
        {
            // Validate input
            if (string.IsNullOrWhiteSpace(jobSeekerId))
                throw new ArgumentException("Job seeker ID cannot be empty", nameof(jobSeekerId));
            ...
            string query = @"UPDATE JobSeeker 
                   SET IsEmailVerified = 1, 
                       ...
                       VerificationAttempts = 0,
                       LastVerificationAttempt = NULL,
                       EmailVerifiedDate = GETUTCDATE(),  -- Added to track when email was verified
                       LastUpdatedDate = GETUTCDATE()     -- Added to track last update
            ...
            try
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    if (rowsAffected == 0)
                    {
                        throw new InvalidOperationException("No user was updated. JobSeeker_Id may not exist.");
                    }
                }
            }
            catch (SqlException ex)
            {
                ...
                LogError($"Error activating user {jobSeekerId}: {ex.Message}");
